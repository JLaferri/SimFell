...
public static class APLParser
{
    ...
    private static SimAction ParseLine(string line, Unit unit)
    {
        ...
        if (!remaining.StartsWith("/"))
        {
            ...
            Console.WriteLine(
                $"WARNING: APL action type '{actionType}' is not a spell action and will be skipped: {trimmed}");
            return null;
        }
        ...
        Func<bool, bool, bool> combineFunc, string originalLine)
    {
        ...
        var parts = conditionText.Split(new[] { operatorText },
    ...
    private static Func<Unit, bool> HandleSpellCondition(string conditionText, string originalLine = null)
    {
        ...
        return unit =>
        {
            ...
            return propertyKey switch
            {
                "cooldown" => CompareValues(((spell.OffCooldown) - unit.Simulator.Now).TotalSeconds,
                    propertyPartWithComparison),
    ...
    private static Func<Unit, bool> HandleSimpleCondition(string conditionText, string originalLine = null)
    {
        ...
        {
            return CreateErrorCondition($"Simple condition must include comparison operator: {conditionText}",
                originalLine);
        }
        ...
        return propertyNameKey switch
        {
            ...
            _ => CreateErrorCondition($"Unknown simple condition property '{propertyNameKey}' in '{conditionText}'",
                originalLine)
        };
    }
    ...
    private static Func<Unit, bool> HandleCharacterCondition(string conditionText, string originalLine = null)
    {
        ...
        else
        {
            ...
            return CreateErrorCondition($"Character condition must include comparison operator: {conditionText}",
                originalLine);
        }
    ...
    private static Func<Unit, bool> CreateUnknownPropertyErrorCondition(string propertyPart, string conditionText,
        string originalLine)
    {
        Console.WriteLine(
